...
var rangeArray = [];              //new arr for results
var maxValue = (x > y) ? x : y;   //maxvalue array
var minValue = (y > x) ? x : y;   //minvalue array
  if(x === y){
    return rangeArray;              //return an empty arr if no num between range
  }
  if(minValue + 1 === maxValue){    //accept larget start than end
    return rangeArray;
  }
  rangeArray.push(minValue + 1);    //case edge for higher start than end
  if(x > y){                        //reverse to get range big start over end num
